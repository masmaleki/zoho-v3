...
class ZohoPurchaseOrderController
{
    ...
    public static function getByItemName($item_name, $organization_id = null)
    {
        ...
        $apiURL = config('zoho-v3.books_api_base_url') . '/api/v3/purchaseorders?page=1&per_page=200&sort_column=created_time&sort_order=D&custom_field_222119000000940129_contains=' . $item_name . '';
        if ($organization_id) {
            $apiURL .= '&organization_id=' . $organization_id;
        }
        $client = new Client();
